...
export default function Switch({
  ...
  modeChangeHabdler?: () => void;
}): JSX.Element {
  //? extract value from local storage:
  const theme = localStorage.getItem("darkMode");

  //? set proper state and initialize localStorage value
  const [themeState, setThemeState] = useState(theme === "false");

  //? trigger modeChangeHabdler for mode change, setThemeState(!themeState) for switch to work
  const toggleTheme = () => {
    setThemeState(!themeState);
    modeChangeHabdler && modeChangeHabdler();
  };
  ...
  return (
    <>
      <label
        ...
        className="themeSwitcherTwo absolute inline-flex cursor-pointer select-none items-center justify-center rounded-md bg-gray-100 dark:bg-gray-900 p-1 dark:border-gray-700 border"
      >
        <input
          type="checkbox"
        ...
        <span
          className={
            themeState
              ? `${darkClassName.join(
                  " "
                )} "light text-primary text-black font-semibold bg-gray flex items-center space-x-[6px] rounded py-2 px-[18px] text-xs font-medium"`
              : "light text-primary bg-gray flex items-center space-x-[6px] rounded py-2 px-[18px] text-xs font-semibold  dark:text-white"
          }
        >
          <svg
            width="14"
        ...
        <span
          className={
            themeState
              ? "light text-primary bg-gray font-semibold flex items-center space-x-[6px] rounded py-2 px-[18px] text-xs"
              : `${darkClassName.join(
                  " "
                )} "light text-primary bg-gray font-semibold flex items-center space-x-[6px] rounded py-2 px-[18px] text-xs font-semibold"`
          }
        >
          <svg
            width="14"
